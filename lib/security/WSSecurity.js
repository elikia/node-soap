...
function WSSecurity(username, password, options) {
  ...
  if(options.wsuID!=null){
    this._wsuID=options.wsuID;
  }
  if(options.mustUnderstandSoapPrefix){
    this._mustUnderstandSoapPrefix=options.mustUnderstandSoapPrefix;
  }
}
...
WSSecurity.prototype.toXML = function() {
  ...
  var password, nonce,wsuID,mustUnderstandSoapPrefix;
  ...
  if(this._wsuID){
    wsuID='wsu:Id="'+this._wsuID+'"';
  }else{
    wsuID="wsu:Id=\"SecurityToken-" + created +"\"";
  }
  if(this._mustUnderstandSoapPrefix){
    mustUnderstandSoapPrefix=this._mustUnderstandSoapPrefix;
  }else{
    mustUnderstandSoapPrefix="soap";
  }

  return "<wsse:Security " + (this._actor ? "soap:actor=\"" + this._actor + "\" " : "") +
    (this._mustUnderstand ? mustUnderstandSoapPrefix+":mustUnderstand=\"1\" " : "") +
    "xmlns:wsse=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd\" "+(wsuID?"":"xmlns:wsu=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\"")+">" +
    timeStampXml +
    "<wsse:UsernameToken xmlns:wsu=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-utility-1.0.xsd\" "+wsuID+ ">" +
